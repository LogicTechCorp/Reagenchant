...
public class ExportDefaultReagentPacksCommand
{
    public static ArgumentBuilder<CommandSource, ?> register()
    {
        return Commands.literal("exportDefaultReagentPacks")
                .requires(source -> source.hasPermissionLevel(2))
                .executes(ExportDefaultReagentPacksCommand::exportReagentPacks);
    }

    private static int exportReagentPacks(CommandContext<CommandSource> context)
    {
        ...
        try
        {
            ModFile modFile = ModList.get().getModFileById(Reagenchant.MOD_ID).getFile();
        ...
        {
            Reagenchant.LOGGER.warn("Unable to export default reagent packs.");
        }
